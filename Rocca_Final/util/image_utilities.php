...
Class ImageUtilities {
    ...
    public static function GetBaseImagesList($dir) {
        ...
        //proccess to get just the files
        $images = array();
        foreach(scandir($dir) as $file) {
            $ext = pathinfo($file, PATHINFO_EXTENSION);

            //make sure that a file is a .jpg or .png type
            if (is_file($dir . $file) &&
                ($ext ==='png' || $ext === 'jpg')) {
                $images[] = $file;
            }
    ...
    //resize the image to fir within the $max x $max square
    private static function ResizeImage($orig, $type, $max) {
        //get the original image, along with its height and width
        $origImage = '';
    ...
    public static function ProcessImage($file) {
        ...
        //based on the image type, create the files
        switch($imgType) {
            case IMAGETYPE_PNG:
